...
class Migros extends Generic {
  ...
  changeLogoLink() {
    $(".logo")
      .closest("a")
      .attr("href", "https://www.migros.ch/de/einkaufen.html");
    $("#skip-navmain a").attr(
      "href",
      "https://www.migros.ch/de/einkaufen.html"
    );
    $(".mui-breadcrumb li:first a").attr(
      "href",
  ...
  getCategoryString(customBody = false) {
    ...
    if (!this.products[GTIN].categoryString) {
      this.products[GTIN].categoryString = $(customBody || "body")
        .find(".mui-breadcrumb li")
        .map(function () {
  ...
  getCategoryStringLinks(customBody = false) {
    ...
    if (!this.products[GTIN].categoryStringLinks) {
      this.products[GTIN].categoryStringLinks = $(".mui-breadcrumb-link")
        .map(function () {
  ...
  getProductCategory(customBody = false) {
    ...
    if (
      !indexOfMany(cat, ["alimentari", "lebensmittel", "denrées alimentaires"])
    ) {
      return "unknown";
    }
  ...
  async changePrice(
    ...
    let currentPrice_chf = currentPriceEl
      .text()
      .replace(".-", "")
      .replace("CHF", "")
      .replace("€", "")
    ...
    if (userCountry === "de") currentPriceEl.text("€ " + currentPrice_eur);
    else currentPriceEl.text("CHF " + currentPrice_chf);

    currentPriceEl.addClass("updated");

    let usualPrice_chf = usualPriceEl
      .text()
      .replace("statt", "")
      .replace(".-", "")
      .replace("CHF", "")
    ...
    if (usualPrice_chf && settings.showDiscount) {
      ...
      const percent = ((1 - currentPrice_chf / usualPrice_chf) * 100).toFixed(
        0
      );

      if (userCountry == "de") usualPriceEl.text("statt " + usualPrice_eur);
      ...
      discountContainer.html(
        $("<div>")
          .css({
            background:
              "linear-gradient(130deg, #ffb696,#ff4d00, #ff4d00, #ffb696)",
            fontSize: "35px",
            color: "white",
    ...
    const getValue = (key) => {
      ...
      const txt = $(customBody || "body")
        ...
        .filter(function () {
          ...
          for (let a of key) {
            if (
              $(this).text().trim().toLowerCase().indexOf(a.toLowerCase()) >= 0
            )
              return true;
          }
          return false;
  ...
  async getProductData(customBody = false) {
    ...
    const sizeMatch =
      regex.exec($body.find(".sidebar-subtext").text()) ||
      regex.exec($body.find(".sidebar-product-name").first().text());
    return {
      category,
      name: $body
        .find(".sidebar-product-name")
        .first()
        .text()
        .replace(regex, ""),
      price,
  ...
  clean() {
    ...
    $(
      ".sidebar-availability-store-information, .sidebar-product-availability"
    ).remove();
